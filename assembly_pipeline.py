...
_DEPENDENCIES = ['fastqcheck', 'spades.py', 'improve_assembly', 'quast.py', 'seqkit']
...
def check_arguments(args):
    ...
    improved_assembly_final = args.sample_id + ".spades.improved.fasta"
...
def run_seqkit_stats(fastq_file):
    """ This function runs seqkit stats command on fastq and returns the maximum read length """
    seqkit_stats = subprocess.Popen(
        ['seqkit', 'stats', fastq_file],
        stdout=subprocess.PIPE,
    )
    end_of_pipe = seqkit_stats.stdout

    read_length = "NA"

    # example of seqkit stats output
    # file format type num_seqs sum_len min_len avg_len max_len
    # ERR5862185_1.fastq FASTQ DNA 188,998 52,339,572 35 276.9 301
    for line in end_of_pipe:
        if "file" not in line.decode('utf-8').strip():
            # removing multiple spaces
            new_line = ' '.join(line.decode('utf-8').strip().split())
...
def _main():
    ...
    if not read_length1.isnumeric():
        logging.warning(f'Read length could not be extracted from {args.forward_reads} using fastqcheck.')
        logging.info(f'Running seqkit stats on {args.forward_reads} instead')
        read_length1 = run_seqkit_stats(args.forward_reads)
        if not read_length1.isnumeric():
    ...
    if not read_length2.isnumeric():
        logging.warning(f'Read length length could not be extracted from {args.reverse_reads}!')
        logging.info(f'Running seqkit stats on {args.reverse_reads} instead')
        read_length2 = run_seqkit_stats(args.reverse_reads)
        if not read_length2.isnumeric():
            logging.error(f'Read length could not be extracted from {args.reverse_reads}')
            sys.exit(-1)
    ...
    # Coping final files
    spades_assembly_final = args.results_dir + "/" + args.sample_id + ".spades.contigs.fasta"
